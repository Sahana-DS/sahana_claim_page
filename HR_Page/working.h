...
<body>
    ...
    <script>
        ...
        async function updateTables() {
            try {
                const response = await fetch('http://54.166.5.133:3407/api/claims');
        ...
        async function updateClaimStatus(claimId, status) {
            ...
            try {
                const response = await fetch(`http://54.166.5.133:3407/api/claims/${claimId}`, {
                    method: 'PATCH',
                    headers: {
        ...
        async function viewClaim(claimId) {
            try {
                const [claimResponse, documentsResponse] = await Promise.all([
                    fetch(`http://54.166.5.133:3407/api/claims?claim_id=${claimId}`),
                    fetch(`http://54.166.5.133:3407/api/claims/${claimId}/documents`)
                ]);
...
`;
        ...
        async function downloadDocument(documentId, fileName) {
            try {
                ...
                const response = await fetch(`http://54.166.5.133:3407/api/documents/${documentId}`);
                ...
                const url = `http://54.166.5.133:3407/api/documents/${documentId}`;
